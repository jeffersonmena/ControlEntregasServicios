...
$selectTea=ejecutarSQL::consultar("SELECT * FROM docente WHERE DNI='".$primaryKey."'");
...
if(mysql_num_rows($selectStudents)<=0){
    $selectAllLoansT=ejecutarSQL::consultar("SELECT * FROM prestamodocente WHERE DNI='".$primaryKey."'");
    $totalP=0;
    ...
    if($totalP<=0){
        $totalErrors=0;
        $selectAllLoansTe=ejecutarSQL::consultar("SELECT * FROM prestamodocente WHERE DNI='".$primaryKey."'");
        ...
        if($totalErrors<=0){
            if(consultasSQL::DeleteSQL("bitacora", "CodigoUsuario='".$primaryKey."' AND Tipo='Docente'")){
                if(consultasSQL::DeleteSQL("docente", "DNI='".$primaryKey."'")){
