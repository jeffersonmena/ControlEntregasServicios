...
$selectAllLoansP=ejecutarSQL::consultar("SELECT * FROM prestamopersonal WHERE DNI='".$primaryKey."'");
$totalP=0;
...
if($totalP<=0){
    $totalErrors=0;
    $selectAllLoansP2=ejecutarSQL::consultar("SELECT * FROM prestamopersonal WHERE DNI='".$primaryKey."'");
    ...
    if($totalErrors<=0){
        if(consultasSQL::DeleteSQL("bitacora", "CodigoUsuario='".$primaryKey."' AND Tipo='Personal'")){
            if(consultasSQL::DeleteSQL("personal", "DNI='".$primaryKey."'")){
